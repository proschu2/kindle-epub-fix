...
export class EPUBBook {
    ...
    async readEPUB(blob) {
        ...
        for (const entry of this.entries) {
            ...
            const ext = filename.split(".").pop();
            if (
                filename === "mimetype" || ["html", "xhtml", "htm", "xml", "svg", "css", "opf", "ncx"].includes(
                    ext
                )
            ) {
...
export async function processEPUB(inputBlob, name) {
    try {
        // Load EPUB
        ...
        if (epub.fixedProblems.length > 0) {
            dlfilenames.push("[FIX] " + name);
        } else {
            dlfilenames.push("[PACKED] " + name);
        }
    } catch (e) {
        console.error(e);
        const idx = filenames.length;
